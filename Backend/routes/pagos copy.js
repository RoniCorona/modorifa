...
const upload = require('../utils/upload'); // <-- ¡IMPORTACIÓN AÑADIDA!
...
// @desc    Registrar un nuevo pago (inicialmente "pendiente") con tickets aleatorios y comprobante
// @route   POST /api/pagos
// @access  Public (Debería ser pública si los clientes registran sus pagos)
router.post('/', upload.single('comprobante'), async (req, res) => { // <-- ¡MIDDLEWARE DE MULTER AÑADIDO!
    console.log('--- Recibiendo solicitud POST /api/pagos ---');
    console.log('Body recibido:', req.body);
    console.log('Archivo recibido (si aplica):', req.file); // req.file contendrá la información del archivo
    ...
    // La URL del comprobante se obtiene de req.file.path si el archivo fue subido
    // Multer guarda en 'uploads/', y Express sirve desde '/uploads',
    // así que la URL relativa para la DB será /uploads/nombre_del_archivo.ext
    const comprobanteUrl = req.file ? `/uploads/${req.file.filename}` : null; // <-- ¡OBTENCIÓN DE LA URL!
    ...
    // Nueva validación: asegúrate de que haya una referencia o un comprobante (o ambos)
    if (!referenciaPago && !comprobanteUrl) {
        console.error('ERROR: Ni referencia de pago ni comprobante proporcionados.');
        // Considera si quieres que el comprobante sea obligatorio para todos los métodos.
        // Si no, ajusta esta validación. Para este caso, lo hacemos obligatorio.
    ...
    try {
        ...
        console.log('Buscando tickets disponibles para la rifa...');
        const ticketsDisponiblesDb = await Ticket.find({
            rifaId: rifaId,
            estado: 'disponible'
        }).select('numeroTicket _id'); 
        console.log(`Encontrados ${ticketsDisponiblesDb.length} tickets en estado 'disponible'.`);

        if (ticketsDisponiblesDb.length < cantidadTickets) {
            console.error('ERROR: No hay suficientes tickets disponibles en DB.');
            return res.status(400).json({ message: `No hay suficientes tickets disponibles en esta rifa. Solo quedan ${ticketsDisponiblesDb.length}.` });
        }

        const numerosTicketsAsignados = [];
        const ticketsIdsParaActualizar = []; 
        
        // --- MODIFICACIÓN CLAVE: Barajar los tickets disponibles antes de seleccionarlos ---
        const ticketsMezclados = shuffleArray([...ticketsDisponiblesDb]);
        console.log('Tickets disponibles barajados. Seleccionando de la lista mezclada.');
        
        for (let i = 0; i < cantidadTickets; i++) {
            // Ahora simplemente tomamos el primer elemento de la lista mezclada y lo removemos
            const selectedTicket = ticketsMezclados.shift(); // .shift() remueve y retorna el primer elemento
            numerosTicketsAsignados.push(selectedTicket.numeroTicket); 
            ticketsIdsParaActualizar.push(selectedTicket._id); 
            // Ya no es necesario splice porque shift ya elimina el elemento
        }
        console.log('Tickets seleccionados para asignación:', numerosTicketsAsignados);

        // --- Lógica para calcular y almacenar montoTotalUSD, montoTotalVES y tasaCambioUsada ---
        let montoEnUSD = 0;
        let montoEnVES = 0;
        let tasaDeCambioActual = rifaExistente.tasaCambio; // Usamos la tasa de cambio de la rifa
        ...
        const nuevoPago = new Pago({
            ...
            comprobanteUrl: comprobanteUrl, // <-- ¡ASIGNACIÓN DE LA URL DEL COMPROBANTE!
            comprador: { 
        ...
        // Actualizar el estado de los tickets seleccionados a 'pendiente_pago'
        console.log('Actualizando estado de tickets a pendiente_pago...');
        ...
        if (updateResult.modifiedCount !== cantidadTickets) {
            ...
            // Considera añadir lógica para revertir la creación del pago o notificar al admin
        }

        // Actualizar el contador de tickets vendidos en la Rifa
        console.log('Actualizando contador de tickets vendidos en la Rifa...');
        ...
        res.status(201).json({
            ...
            pago: pagoGuardado,
            numerosTicketsAsignados: pagoGuardado.numerosTicketsAsignados,
            ...
            comprobanteUrl: comprobanteUrl, // <-- ¡ENVIANDO LA URL DEL COMPROBANTE AL FRONTEND!
            rifaActualizada: {
        ...
        if (err.code === 11000) { 
            console.error('ERROR: Error de duplicado (11000) - referenciaPago ya existe.', err);
            return res.status(409).json({ message: 'Ya existe un pago con esta referencia bancaria. Por favor, verifica tu información o contacta a soporte.', field: 'referenciaPago' });
        }
...
router.patch('/:id/rechazar', protect, authorize(['admin']), async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Pago rechazado exitosamente', pago: pagoActualizado });
    } catch (err) {
...
router.delete('/:id', protect, authorize(['admin']), async (req, res) => {
    try {
        ...
        // Liberar los tickets
        // Esto se aplica si el pago estaba 'verificado' o 'pendiente'.
        // Si estaba 'rechazado', los tickets ya se habrían liberado.
        ...
        // --- MODIFICACIÓN CLAVE: Actualizar el contador de tickets vendidos en la Rifa ---
        // Solo decrementamos los tickets vendidos si el pago estaba en un estado que los consideraba vendidos (verificado o pendiente).
        // Si el pago estaba 'rechazado', los tickets ya se habrían revertido.
        if (pago.estado === 'verificado' || pago.estado === 'pendiente') {
            const rifaActualizada = await Rifa.findByIdAndUpdate(
                rifaId,
                { $inc: { ticketsVendidos: -cantidadTicketsEliminados } }, // Decrementa los tickets vendidos
                { new: true } 
            );
        ...
        res.status(200).json({ message: 'Pago eliminado exitosamente y tickets de rifa revertidos.' });
    } catch (err) {
